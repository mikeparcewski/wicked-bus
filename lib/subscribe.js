 ...
 * the ghost shm node anyway); what it CAN do is name the condition — WB-014, with a counter — so the
 * consumer can tell "this connection is dead" from "this poll failed" and act.
 */
      ...
      if (isConnectionFatal(err)) {
        ...
        safeCallback(opts.onError, new WBError('WB-014', 'SUBSCRIBER_DB_UNUSABLE', {
          message:
            ...
            'database is unusable and re-polling will not recover it: reopen the connection ' +
            '(a second SQLite library on the same file in this process, or WAL sidecars unlinked ' +
            'under open connections, are the known causes)',
