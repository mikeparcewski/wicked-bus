...
/**
 * Re-anchor a cursor's durable position to a new floor.
 *
 * Used to recover a cursor that has fallen behind the TTL sweep window
 * (WB-003). The swept events are gone and unrecoverable regardless; the only
 * way to resume delivery is to move the durable `last_event_id` forward to the
 * oldest position that still lets the next poll return every surviving event.
 *
 * The caller re-anchors to `oldest_available_event_id - 1` so the next poll
 * (`event_id > last_event_id`) delivers from the oldest surviving event — it
 * does NOT fast-forward to the stream head, so no still-deliverable event is
 * skipped and at-least-once is preserved for everything that survived the sweep.
 *
 * @param {import('better-sqlite3').Database} db
 * @param {string} cursorId
 * @param {number} newLastEventId - The floor to set (typically MIN(event_id) - 1)
 * @returns {{ reanchored: boolean, cursor_id: string, last_event_id: number }}
 */
export function reanchorCursor(db, cursorId, newLastEventId) {
  // Public API: reject a bad position before it corrupts cursors.last_event_id
  // (a NaN/negative would break every future poll + WB-003 comparison).
  if (!Number.isInteger(newLastEventId) || newLastEventId < 0) {
    throw new WBError('WB-001', 'INVALID_EVENT_SCHEMA', {
      message: `reanchorCursor: newLastEventId must be a non-negative integer, got: ${newLastEventId}`,
      reason: 'invalid cursor position',
      cursor_id: cursorId,
    });
  }
  ...
  const txn = db.transaction(() => {
    const result = update.run(newLastEventId, now, cursorId);
    if (result.changes === 0) {
      throw new WBError('WB-006', 'CURSOR_NOT_FOUND', {
        message: 'Cursor not found or deregistered',
        cursor_id: cursorId,
