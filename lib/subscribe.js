 ...
 * - A cursor the TTL sweep left behind (WB-003) is RE-ANCHORED by the loop to
 *   `MIN(event_id) - 1` — the oldest surviving event, never the head — and the
 *   error is reported once per subscriber per sweep with `reanchored_to` and
 *   `swept_past`; delivery resumes on the next tick (wicked-crew #445; the
 *   swept rows were unrecoverable regardless).
 *
 * Caveats:
...
import { poll, ack, reanchorCursor } from './poll.js';
import { WBError } from './errors.js';
import { register } from './register.js';

/**
 * SQLite result codes that mean THIS handle's view of the database is gone for good — not that one
 * poll failed: the pages it reads are inconsistent from where it stands (`SQLITE_CORRUPT`), the
 * file is not a database (`SQLITE_NOTADB`), or the I/O underneath it failed (`SQLITE_IOERR*`).
 * Re-polling the same handle cannot recover. Seen in the wild (wicked-crew F-E2E-021): six
 * subscribers looped on `database disk image is malformed` every 2 s for hours after a SECOND
 * SQLite library in the same process let an external close unlink the WAL sidecars under them.
 * The caller owns the handle, so the loop cannot reopen it (and a same-library reopen would inherit
 * the ghost shm node anyway); what it CAN do is name the condition — WB-014, once per poll tick with a
 * running counter — so the consumer can tell "this connection is dead" from "this poll failed" and
 * act on the one safe remediation (see REMEDIATION below).
 */
const CONNECTION_FATAL_SQLITE = /^SQLITE_(CORRUPT|NOTADB|IOERR)/;

/**
 * The ONLY safe remediation for WB-014, carried in `context.remediation` and the message. Proven
 * (tests/unit/subscribe-ghost-close-corrupts.test.js): a same-library in-process reopen inherits the
 * ghost WAL index and fails identically, and a graceful `close()` of the LAST ghost connection takes
 * the EXCLUSIVE lock, checkpoints the GHOST WAL over pages other writers already advanced, and leaves
 * bus.db failing `integrity_check` on disk — while `process.exit` without close (the kernel drops the
 * fds; no checkpoint-on-close) leaves the file intact.
 */
const REMEDIATION = 'exit this process WITHOUT closing its bus connections, then restart it';

function isConnectionFatal(err) {
  return typeof err?.code === 'string' && CONNECTION_FATAL_SQLITE.test(err.code);
}
 ...
 *
 * @returns {{ stop: () => Promise<void>, getLag: () => object, getHealth: () => object, cursor_id: string, subscription_id: string }}
 */
export function subscribe(opts) {
  ...
  // WB-014 state: consecutive polls that failed with a connection-fatal SQLite code (see
  // CONNECTION_FATAL_SQLITE). Reset by the next poll that succeeds.
  let consecutiveUnusablePolls = 0;
  let lastUnusablePoll = null;
  ...
  async function tick() {
    ...
    try {
      ...
      consecutiveUnusablePolls = 0;
      lastUnusablePoll = null;
      ...
      if (isConnectionFatal(err)) {
        // This handle's view of the db is unusable (WB-014): say so, with a counter, instead of
        // surfacing the raw driver error every tick as if it were transient. The cadence is
        // unchanged — the consumer decides how loudly to relay a repeat.
        consecutiveUnusablePolls += 1;
        lastUnusablePoll = { at: Date.now(), sqlite_code: err.code, message: err.message };
        safeCallback(opts.onError, new WBError('WB-014', 'SUBSCRIBER_DB_UNUSABLE', {
          message:
            `subscriber ${opts.plugin} cannot read the bus db (${err.code}: ${err.message}); ` +
            `${consecutiveUnusablePolls} consecutive poll(s) failed — this connection's view of the ` +
            'database is unusable and re-polling will not recover it. Remediation: ' +
            REMEDIATION + '. Do NOT reopen in-process (a same-library reopen inherits the ghost ' +
            'WAL index) and do NOT close() the affected connections (a graceful close of the last ' +
            'one checkpoints the ghost WAL into the file and corrupts it on disk). Known causes: a ' +
            'second SQLite library on the same file in this process; WAL sidecars unlinked under ' +
            'open connections.',
          remediation: REMEDIATION,
          sqlite_code: err.code,
          sqlite_message: err.message,
          consecutive: consecutiveUnusablePolls,
          plugin: opts.plugin,
          subscription_id,
          cursor_id,
          db_path: db.name,
        }), null);
      } else if (err instanceof WBError && err.error === 'WB-003') {
        // WB-003 CURSOR_BEHIND_TTL_WINDOW (wicked-crew #445): the TTL sweep removed rows this
        // cursor never acked. Re-polling the same position can never succeed — the rows are gone —
        // so before 2.3.5 a subscriber that fell behind sat here FOREVER, wedged on the same error
        // every tick (crew's seven subscribers, and the document veil that never resolved). This is
        // the ONE place that holds both the handle and the cursor, so it re-anchors: `last_event_id`
        // moves to `oldest_available_event_id - 1`, the floor `reanchorCursor` documents — the next
        // tick delivers from the OLDEST SURVIVING event, never the head, so at-least-once holds for
        // everything the sweep left. The error is still reported, ONCE per subscriber per sweep,
        // with what happened: `reanchored_to`, and `swept_past` — the row range that is gone (how
        // many of those rows matched this subscriber's filter is unknowable; a filtered subscriber
        // that matched nothing lands here too, having lost nothing, because cursors advance only
        // on ack).
        const floor = err.context.oldest_available_event_id - 1;
        try {
          reanchorCursor(db, cursor_id, floor);
        } catch (writeErr) {
          // The re-anchor is a WRITE (SQLITE_BUSY is possible): report it as its own error and let
          // the next tick try again — it must never escape tick() as an unhandled rejection.
          safeCallback(opts.onError, writeErr, null);
          return;
        }
        err.context.reanchored_to = floor;
        err.context.swept_past = [err.context.cursor_last_event_id + 1, floor];
        err.context.remediation =
          'rows in swept_past were removed by the TTL sweep before this subscriber acked them ' +
          '(how many matched its filter is unknowable); the cursor now sits at the oldest ' +
          'surviving event — delivery resumes on the next tick';
        safeCallback(opts.onError, err, null);
      } else {
        // Other polling errors (WB-006, SQLITE_BUSY, …) bubble through onError so operators
  ...
  /** WB-014 health: is this handle's view of the db unusable, and since when. */
  function getHealth() {
    return {
      unusable: consecutiveUnusablePolls > 0,
      consecutive_unusable_polls: consecutiveUnusablePolls,
      last_unusable_poll: lastUnusablePoll ? { ...lastUnusablePoll } : null,
    };
  }
  ...
  return { stop, getLag, getHealth, cursor_id, subscription_id };
}
