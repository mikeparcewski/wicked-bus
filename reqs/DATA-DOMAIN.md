...
| `domain` | TEXT | No | The wicked-* domain that emitted the event. Max 64 chars. Example: `wicked-testing`, `wicked-garden`, `wicked-brain`. Used with `subdomain` to identify which publisher and functional area emitted the event (e.g. `wicked.test.project.created` from `wicked-testing` vs. `wicked.crew.project.created` from `wicked-crew`). |
