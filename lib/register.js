...
export function deregisterByPlugin(db, plugin, opts = {}) {
  // Public API: validate inputs so a bad caller gets a structured WB-001 instead
  // of a silent empty query that misreports as WB-006 ("no active subscription").
  if (typeof plugin !== 'string' || plugin.length === 0) {
    throw new WBError('WB-001', 'INVALID_EVENT_SCHEMA', {
      message: `deregisterByPlugin: plugin must be a non-empty string, got: ${plugin}`,
      reason: 'invalid plugin',
    });
  }
  if (opts.role != null && opts.role !== 'subscriber' && opts.role !== 'provider') {
    throw new WBError('WB-001', 'INVALID_EVENT_SCHEMA', {
      message: `deregisterByPlugin: role must be subscriber|provider, got: ${opts.role}`,
      reason: 'invalid role',
    });
  }
