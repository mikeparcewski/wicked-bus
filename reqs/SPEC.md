...
> **Currency note (2026-08-30):** the tables below are **generated** from the
> producers' machine-readable seams (event registries, event-type constants,
> emit call sites) by `gen_event_catalog.py` in wicked-core
> (`crates/wicked-governance/seed/`, AW-21). Do not hand-edit rows — edit the
> producing seam (or the annotations file next to the script) and regenerate;
> `--check` is the staleness gate, `--drift` the declared-vs-emitted query
> (report also embedded in `wicked-core/EVENTS.md`). The original v1 catalog
> predated the wicked-testing and wicked-brain retirements (2026-08). <!-- historical -->

Events are grouped by the grammar's `<domain>` segment. The `wicked.test.*`
lifecycle types are a **legacy-stable namespace** kept by decision at the
wicked-testing retirement — the types kept their names; the `domain` column <!-- historical -->
stamp is `qe`.

<!-- BEGIN GENERATED EVENT-CATALOG TABLES (AW-21) — generated by wicked-core crates/wicked-governance/seed/gen_event_catalog.py; do not hand-edit between the markers, edit the producing seam or event-catalog-annotations.json and regenerate -->
...
<!-- END GENERATED EVENT-CATALOG TABLES (AW-21) -->
