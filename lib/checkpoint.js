...
export function startCheckpoint(db, config) {
  // Coerce before deciding: a hand-edited config.json can carry a STRING (e.g. "0"), and a
  // truthy "0" would otherwise slip past the disable check into `"0" * 60_000 = 0` — a 0ms
  // setInterval spin. Non-finite garbage disables too (fail safe, never a tight loop).
  ...
  // Floor the tick at 1s so a tiny positive fraction (0.001 minutes) can't spin either.
