...
export function openDb(config = {}) {
  ...
  // Auto-checkpoint every 512 pages (≈2MB at the 4KB default page size) instead of SQLite's
  // 1000-page default — the backstop that keeps bus.db-wal from outgrowing bus.db between the
  // periodic TRUNCATE checkpoints (lib/checkpoint.js). Passive auto-checkpoints defer while
  // readers ride the WAL, so a tighter bound matters on a bus with long-lived pollers.
